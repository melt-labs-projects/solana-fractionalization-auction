...
///   6. `[]` Token program
///   7. `[]` Rent sysvar
const initVaultInstruction = (fractionMint, redeemTreasury, fractionTreasury, vaultAccount, vaultAuthority, priceAccount, data) => {
    return new web3.TransactionInstruction({
        keys: [
...
const addTokenToInactiveVaultInstruction = (safetyBoxPDA, tokenAccount, tokenStoreAccount, vaultAccount, vaultAuthority, payer, transferAuthority, data) => {
    return new web3.TransactionInstruction({
        keys: [
...
///   4. `[signer]` Authority on the vault
///   5. `[]` Token program
const activateVault = (vaultAccount, fractionMint, fractionTreasury, vaultPDA, vaultAuthority, data) => {
    return new web3.TransactionInstruction({
        keys: [
            { pubkey: vaultAccount.publicKey, isSigner: false, isWritable: true, },
            { pubkey: fractionMint.publicKey, isSigner: false, isWritable: true, },
...
///   4. `[]` Token program
///   5. `[]` Rent sysvar
const withdrawSharesFromTreasury = (destAccount, fractionTreasury, vaultAccount, vaultPDA, vaultAuthority, data) => {
    return new web3.TransactionInstruction({
        keys: [
